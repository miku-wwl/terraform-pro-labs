...
本轮是 Phase 7：Core Authoring Batch B。
...
- Lab 20：JSON、CSV、jsondecode、csvdecode、数据整形和稳定 key。
- Lab 21：dynamic nested blocks，避免静态重复。
- Lab 22：create_before_destroy 的真实 replacement 语义。
- Lab 23：ignore_changes 的正确边界，不能让 Terraform 忽略所有有意义的 drift。

特别要求：

- 文件输入使用确定性 fixtures；
- 测试 dynamic block 的数量和内容；
- lifecycle 测试应验证 configuration/plan 行为；
- ignore_changes 只能覆盖外部系统拥有的字段；
- starter 不得通过删除 lifecycle 测试蒙混过关。

完成 starter、solution、reset 和回归后停止。
