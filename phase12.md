...
本轮是 Phase 12：State and Refactor Batch B。
...
- Lab 12：remote-state consumer 和 backend separation。
- Lab 13：workspace-aware behavior 和 production guardrails。
- Lab 19：count 到 for_each 的 state-preserving refactor。
- Lab 26：moved、removed block 和 destroy=false 语义。

要求：

- Lab 19 必须验证旧 index 到新 logical key 的精确映射；
- Lab 26 必须区分 removed block 与 moved block；
- removed block 的 destroy 语义必须准确；
- 不允许通过删除 state 伪造 no-op；
- 每个 refactor 题都要保留 refactor 前 fixture；
- 检查最终 state list 和 plan actions；
- reset 后执行第二遍。

完成四题以及全部 state/refactor labs 回归后停止。
